import { optimizarCortes } from './remisiones.js';
...
function renderDespieceUI() {
    ...
    <div class="w-full max-w-[98%] mx-auto space-y-6 px-1 md:px-4">
        <div>
            <h1 class="text-2xl font-extrabold text-slate-900 tracking-tight">Optimizador de Despiece 2D</h1>
            <p class="text-xs font-semibold text-slate-450 mt-1">Calcula el corte óptimo de láminas de vidrio con el algoritmo de empaquetamiento 2D e impresión de planos.</p>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="space-y-6 lg:col-span-1">
                <div class="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm space-y-4">
                    <h2 class="text-md font-bold text-slate-800 flex items-center gap-2">📐 Lámina Base</h2>
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <label for="sheet-width" class="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Ancho (mm)</label>
                            <input type="number" id="sheet-width" class="w-full mt-1 p-2.5 border rounded-lg text-sm" value="3300" min="100">
                        </div>
                        <div>
                            <label for="sheet-height" class="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Alto (mm)</label>
                            <input type="number" id="sheet-height" class="w-full mt-1 p-2.5 border rounded-lg text-sm" value="2140" min="100">
                        </div>
                    </div>
                    <div class="pt-1">
                        <label for="sheet-client" class="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Cliente / Proyecto (Opcional)</label>
                        <input type="text" id="sheet-client" class="w-full mt-1 p-2.5 border rounded-lg text-sm" placeholder="Ej: Obra 1 / Cliente X">
                    </div>
                    <div class="pt-1">
                        <label for="sheet-material" class="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Tipo de Material</label>
                        <input type="text" id="sheet-material" class="w-full mt-1 p-2.5 border rounded-lg text-sm" value="LAMINADO 3+3" placeholder="Ej: LAMINADO 3+3">
                    </div>                    <div class="pt-2">
                        <label for="opt-strategy" class="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Tipo de Optimización</label>
                        <select id="opt-strategy" class="w-full mt-1 p-2.5 border rounded-lg text-sm bg-white">
                    ...
                    <div class="space-y-2 pt-2">
                        <label class="flex items-center space-x-2 cursor-pointer my-0">
                            <input type="checkbox" id="opt-rotate" class="h-4 w-4 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500" checked>
                            <span class="text-xs font-semibold text-slate-700">Permitir rotación de piezas</span>
                        </label>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-6 lg:col-span-2">
                <div class="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm space-y-4">
                    <h2 class="text-md font-bold text-slate-800">🧩 Añadir Piezas a Cortar</h2>
                    ...
                    <form id="add-pieza-form" class="grid grid-cols-1 sm:grid-cols-12 gap-3 items-end">
                        <div class="sm:col-span-3">
                            ...
                            <input type="number" id="pieza-width" class="w-full mt-1 p-2.5 border rounded-lg text-sm" placeholder="Ej: 1200" required min="10">
                        </div>
                        <div class="sm:col-span-3">
                            <label for="pieza-height" class="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Alto pieza (mm)</label>
                            <input type="number" id="pieza-height" class="w-full mt-1 p-2.5 border rounded-lg text-sm" placeholder="Ej: 800" required min="10">
                        </div>
                        <div class="sm:col-span-2">
                            <label for="pieza-cant" class="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Cantidad (uds)</label>
                            <input type="number" id="pieza-cant" class="w-full mt-1 p-2.5 border rounded-lg text-sm" value="1" required min="1">
                        </div>
                        <div class="sm:col-span-2">
                            <label for="pieza-label" class="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Etiqueta (Opcional)</label>
                            <input type="text" id="pieza-label" class="w-full mt-1 p-2.5 border rounded-lg text-sm" placeholder="Ej: Ventana">
                        </div>
                        <div class="sm:col-span-2">
                            <button type="submit" class="w-full bg-indigo-650 hover:bg-indigo-755 text-white font-extrabold py-2.5 px-4 rounded-lg text-xs shadow-md transition-all h-[42px] flex items-center justify-center gap-1">
                                <span>+</span> Agregar
                            </button>
                        </div>
                    </form>
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-t border-slate-100 pt-4 pb-2">
                        <span class="text-xs font-bold text-slate-750 flex items-center gap-1">
                            📋 Listado de Piezas a Optimizar
                        </span>
                        <div class="flex gap-2">
                            <button id="download-template-btn" class="bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border border-emerald-250 px-3 py-1.5 rounded-lg text-[11px] font-bold flex items-center gap-1 shadow-sm transition">
                                📊 Plantilla Excel
                            </button>
                            <button id="import-excel-btn" class="bg-indigo-50 hover:bg-indigo-100 text-indigo-700 border border-indigo-250 px-3 py-1.5 rounded-lg text-[11px] font-bold flex items-center gap-1 shadow-sm transition">
                                📥 Importar Excel
                            </button>
                            ...
                            <table class="w-full text-left border-collapse">
                                <thead>
                                    <tr class="border-b border-slate-100 text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                                        <th class="pb-2">Pieza</th>
                                        <th class="pb-2">Etiqueta</th>
                                        <th class="pb-2">Ancho (mm)</th>
                                        <th class="pb-2">Alto (mm)</th>
                    ...
                    <button id="calculate-despiece-btn" class="w-full bg-[#0066e2] text-white font-extrabold py-3.5 px-4 rounded-xl hover:bg-blue-700 transition shadow-md flex items-center justify-center gap-2">
        ...
        <div id="despiece-results-container" class="hidden grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-1 space-y-6">
                <div class="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm space-y-4">
                    <h2 class="text-md font-bold text-slate-800">📊 Resultados del Cálculo</h2>
                    
                    <div class="grid grid-cols-2 gap-4">
                        <div class="bg-slate-50 p-3 rounded-lg border border-slate-100 text-center">
                            <span class="text-[9px] font-bold text-slate-400 uppercase tracking-wider">Láminas Necesarias</span>
                            <p id="res-sheets-count" class="text-2xl font-black text-[#0066e2]">0</p>
                        </div>
                        <div class="bg-slate-50 p-3 rounded-lg border border-slate-100 text-center">
                            <span class="text-[9px] font-bold text-slate-400 uppercase tracking-wider">Aprovechamiento</span>
                            <p id="res-utilization" class="text-2xl font-black text-emerald-600">0%</p>
                        </div>
                    </div>

                    <div class="space-y-2 text-xs font-semibold text-slate-600">
                        <div class="flex justify-between">
                            <span>Total Piezas Cortadas:</span>
                            <span id="res-pieces-placed" class="text-slate-800">0</span>
                        </div>
                        <div class="flex justify-between">
                            <span>Área Útil Cortada:</span>
                            <span id="res-useful-area" class="text-slate-800">0 m²</span>
                        </div>
                        <div class="flex justify-between">
                            <span>Área de Desperdicio:</span>
                            <span id="res-waste-area" class="text-slate-800">0 m²</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="lg:col-span-2 space-y-6">
                <div class="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm space-y-4">
                    <div class="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                        <h2 class="text-md font-bold text-slate-800 flex items-center gap-2">🖼️ Plano 2D de Corte</h2>
                        <div id="sheet-tabs" class="flex gap-1.5 overflow-x-auto whitespace-nowrap max-w-full sm:max-w-[70%] py-1 pb-2 scrollbar-thin"></div>
                    </div>

                    <div class="flex justify-center bg-slate-50 p-4 rounded-xl border border-slate-200/60 overflow-hidden relative min-h-[300px]">
                        <canvas id="despiece-canvas" class="max-w-full h-auto shadow-md bg-white rounded-lg"></canvas>
                    </div>
                    
                    <div class="flex flex-col sm:flex-row gap-3 justify-between sm:items-center text-xs text-slate-450 font-bold">
                        <span class="flex items-center gap-1.5">🛈 Desplaza o haz zoom en la vista para ver los cortes detallados</span>
                        <div class="flex gap-2">
                            <button id="print-labels-btn" class="bg-indigo-50 hover:bg-indigo-150 text-indigo-700 border border-indigo-200 px-3.5 py-2 rounded-lg flex items-center gap-1 shadow-sm transition">
                                🏷️ Imprimir Rótulos (Stickers)
                            </button>
                            <button id="print-all-sheets-btn" class="bg-[#0066e2] hover:bg-blue-700 text-white px-3.5 py-2 rounded-lg flex items-center gap-1 shadow-sm transition">
...
function renderPiezasTable() {
    ...
    if (piezasList.length === 0) {
        tbody.innerHTML = `<tr><td colspan="6" class="py-8 text-center text-gray-400">No hay piezas agregadas.</td></tr>`;
        return;
    }

    tbody.innerHTML = piezasList.map((p, idx) => `
        <tr class="hover:bg-slate-50/50">
            <td class="py-3 font-bold text-[#0066e2]">${idx + 1}</td>
            <td class="py-3">
                <input type="text" class="edit-label-input w-full max-w-[130px] p-1.5 border rounded-lg text-xs bg-slate-50 focus:bg-white focus:ring-1 focus:ring-[#0066e2]" value="${p.label || ''}" data-id="${p.id}" placeholder="Ej: Ventana">
            </td>
            <td class="py-3">
                <input type="number" class="edit-width-input w-20 p-1.5 border rounded-lg text-xs bg-slate-50 focus:bg-white focus:ring-1 focus:ring-[#0066e2]" value="${p.w0}" data-id="${p.id}" min="10">
            </td>
            <td class="py-3">
                <input type="number" class="edit-height-input w-20 p-1.5 border rounded-lg text-xs bg-slate-50 focus:bg-white focus:ring-1 focus:ring-[#0066e2]" value="${p.h0}" data-id="${p.id}" min="10">
            </td>
            <td class="py-3">
                <input type="number" class="edit-cant-input w-20 p-1.5 border rounded-lg text-xs bg-slate-50 focus:bg-white focus:ring-1 focus:ring-[#0066e2]" value="${p.cant}" data-id="${p.id}" min="1">
            </td>
            <td class="py-3 text-right">
                <button class="delete-pieza-btn text-red-500 hover:text-red-700 text-base" data-id="${p.id}" title="Eliminar">🗑️</button>
            </td>
        </tr>
    `).join('');
...
function calculateOptimization() {
    ...
    // Flatten pieces list into cuts array for optimizarCortes ({ ancho, alto, cantidad, label })
    ...
    calculateBtn.innerHTML = '⏳ Calculando Optimización...';
    ...
    setTimeout(() => {
        try {
            ...
            if (!result || !result.plano || result.plano.length === 0) {
                ...
                calculateBtn.innerHTML = '🚀 Calcular Optimización';
                return;
            }
...
function displayResults(sheetW, sheetH) {
    ...
    tabsContainer.innerHTML = currentResult.plano.map((_, idx) => `
        <button class="sheet-tab-btn px-3 py-1.5 rounded-lg text-xs font-bold transition ${idx === currentSheetIndex ? 'bg-[#0066e2] text-white shadow' : 'bg-slate-100 hover:bg-slate-200 text-slate-700'}" data-index="${idx}">
            Lamina ${idx + 1}
        </button>
    `).join('');
    ...
    tabsContainer.querySelectorAll('.sheet-tab-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            tabsContainer.querySelectorAll('.sheet-tab-btn').forEach(b => {
                b.classList.remove('bg-[#0066e2]', 'text-white', 'shadow');
                b.classList.add('bg-slate-100', 'text-slate-700');
            });
            btn.classList.remove('bg-slate-100', 'text-slate-700');
            btn.classList.add('bg-[#0066e2]', 'text-white', 'shadow');
...
function printAllSheets() {
    ...
    win.document.write(`
        <html>
        <head>
            <title>Plano de Despiece Completo - ${clientName || 'Vidrio Express'}</title>
            <style>
                @page {
                    size: landscape;
                    margin: 8mm;
        ...
        <body>
            <div class="main-header">
                <h1>Vidrio Express - Plano General de Despiece 2D</h1>
...
<head>
  ...
  <title>Rotulos Vidrio Express - Lote ${(chunkIdx / chunkSize) + 1}</title>
  <style>
